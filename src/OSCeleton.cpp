...
int PORT = 7110;
int SCRATCH_PORT=42001; // Default port for Scratch messages
...
const int BufLen = OUTPUT_BUFFER_SIZE;
char SendBuf[BufLen];
...
XnSkeletonJoint jointList[] = {
	XN_SKEL_HEAD, XN_SKEL_NECK, XN_SKEL_LEFT_COLLAR, XN_SKEL_LEFT_SHOULDER, XN_SKEL_LEFT_ELBOW, XN_SKEL_LEFT_WRIST,
	XN_SKEL_LEFT_HAND, XN_SKEL_LEFT_FINGERTIP, XN_SKEL_RIGHT_COLLAR, XN_SKEL_RIGHT_SHOULDER, XN_SKEL_RIGHT_ELBOW,
	XN_SKEL_RIGHT_WRIST, XN_SKEL_RIGHT_HAND, XN_SKEL_RIGHT_FINGERTIP, XN_SKEL_TORSO, XN_SKEL_WAIST, XN_SKEL_LEFT_HIP,
	XN_SKEL_LEFT_KNEE, XN_SKEL_LEFT_ANKLE, XN_SKEL_LEFT_FOOT, XN_SKEL_RIGHT_HIP, XN_SKEL_RIGHT_KNEE, XN_SKEL_RIGHT_ANKLE,
	XN_SKEL_RIGHT_FOOT };

char* jointNames[] = { "head", "neck", "l_collar", "l_shoulder", "l_elbow", "l_wrist", "l_hand", "l_fingertip", "r_collar",
	"r_shoulder", "r_elbow", "r_wrist", "r_hand", "r_fingertip", "torso", "waist", "l_hip", "l_knee", "l_ankle", "l_foot",
	"r_hip", "r_knee", "r_ankle", "r_foot" };

void broadcastScratch(char* message)
{
	//---------------------------------------------
	// Before we send each message, we need to send some size code
	unsigned char sizeBytes[4];
	int len = strlen(message);
	// Can't find the example page I found this on - think it was 'Catenary' who posted it(spelling?)
	...
	sendto(SendSocket, message, strlen(message), 0, (struct sockaddr *) &RecvAddr, sizeof(RecvAddr));
}
...
void XN_CALLBACK_TYPE new_user(xn::UserGenerator& generator, XnUserID nId, void* pCookie) {
	...
	if (scratchMode) {
		broadcastScratch("broadcast new_user");
		return;
	}
...
void XN_CALLBACK_TYPE lost_user(xn::UserGenerator& generator, XnUserID nId, void* pCookie) {
	...
	if (scratchMode) {
		broadcastScratch("broadcast lost_user");
		return;
	}
	if (kitchenMode) return;
...
void XN_CALLBACK_TYPE calibration_ended(xn::SkeletonCapability& capability, XnUserID nId, XnBool bSuccess, void* pCookie) {
	if (bSuccess) {
		...
		if (scratchMode) {
			broadcastScratch("broadcast tracking");
			return;
		}
		if (kitchenMode) return;
...
int jointPos(XnUserID player, XnSkeletonJoint eJoint) {
	...
	if (scratchMode) {
		// The -1 is for Scratch
		jointCoords[0] = player;
		jointCoords[1] = -1 * (off_x + (mult_x * (1280 - joint.position.X) / 2560)); //Normalize coords to 0..1 interval
		jointCoords[2] = -1 * (off_y + (mult_y * (1280 - joint.position.Y) / 2560)); //Normalize coords to 0..1 interval
		jointCoords[3] = -1 * (off_z + (mult_z * joint.position.Z * 7.8125 / 10000)); //Normalize coords to 0..7.8125 interval
	} else {
		userID = player;
...
int usage(char *name) {
	printf("\nUsage: %s [OPTIONS]\n\
		   ...
		   -c\t\t Enable \"Scratch\" mode (MIT Scratch compatibility mode).\n\
...
int main(int argc, char **argv) {
	...
	if (oscFunc == NULL)
		oscFunc = genOscMsg;
	if (scratchMode) {
		PORT = SCRATCH_PORT;	// Scratch Specific port
		nDimensions = 3;
		//Multipliers for coordinate system. Scratch values here
		mult_x = 480;
		mult_y = 360;
		mult_z = 1;
		
		//Offsets for coordinate system. Scratch values here
		off_x = -240;
		off_y = -180;
		off_z = 0.0;
	}
	...
	if (scratchMode)
		printf("Scratch (MIT Scratch compatibility)\n");
	...
	if (scratchMode) {
		printf("Broadcast to Scratch\n");
		broadcastScratch("broadcast kinect_server");
	}
	if (preview) {
